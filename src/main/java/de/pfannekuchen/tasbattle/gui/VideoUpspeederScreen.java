...
public class VideoUpspeederScreen extends Screen {
	...
	protected void init() {
		if (isEncoding) {
			addButton(new ButtonWidget(width / 2 - 153, height - 40, 306, 20, new LiteralText("Continue encoding in the background >>"), (b) -> {
				MinecraftClient.getInstance().openScreen(new NewTitleScreen());
			}));
			return;
		}
		
		addButton(new TextFieldWidget(client.textRenderer, (width / 12) * 1 - (width / 24), (height / 8), (width / 12) * 9, 20, new LiteralText(""))).setMaxLength(999);
		addButton(new ButtonWidget((width / 12) * 10 + 5 - (width / 24), (height / 8), (width / 12) * 2, 20, new LiteralText("Select File"), (b) -> {
		...
		addButton(new ButtonWidget((width / 2) - 70, (height / 8) * 2 + 20, 20, 20, new LiteralText("-"), (b) -> {
			if (tickrate != 1) tickrate--;
			((TextFieldWidget) buttons.get(buttons.size() - 1)).setText(tickrate + "");
		}));
		addButton(new ButtonWidget((width / 2) + 50, (height / 8) * 2 + 20, 20, 20, new LiteralText("+"), (b) -> {
			tickrate++;
			((TextFieldWidget) buttons.get(buttons.size() - 1)).setText(tickrate + "");
		}));
		addButton(new ButtonWidget((width / 2) - 98, this.height - (this.height / 10) - 15 - 20 - 5, 204, 20, new LiteralText("Speed up video"), (b) -> {
			b.active = false;
			isEncoding = true;
		...
		addButton(new CheckboxWidget(2, this.height - 22, 20, 20, new LiteralText("High Quality"), false, true));
	...
	public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
		...
		if (isEncoding) {
			...
			drawTextWithShadow(matrices, client.textRenderer, new LiteralText("Est: " + getDuration(est)), i, 40 + (9 + 3) * 2, 10526880);
			drawTextWithShadow(matrices, client.textRenderer, new LiteralText(bitrate + ""), i, 40 + 9 + 3, 10526880);
		...
		if (VideoUpspeederScreen.downloadingFFmpeg) {
			...
			drawCenteredText(matrices, client.textRenderer, new LiteralText(installingProgress), this.width / 2, this.height / 2, 16777215);
